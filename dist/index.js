...
commander_1.default
    ...
    .action(() => {
    const active = child_process_1.execSync('npm get registry').toString().trim();
...
commander_1.default
    ...
    if (REGISTRY.hasOwnProperty(registry)) {
        const result = child_process_1.execSync(`npm set registry ${REGISTRY[registry].registry}`).toString().trim();
